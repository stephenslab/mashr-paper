...
compute.hm.covmat.all.max.step = function(b.hat,se.hat,t.stat,Q,lambda.mat,A,factor.mat,max.step,zero=FALSE,max=1,min=1){
  X.real=as.matrix(t.stat)
  X.c=apply(X.real,2,function(x) x-mean(x)) ##Column centered matrix of t statistics
  R=ncol(X.c)
  omega=mult.tissue.grid(mult=sqrt(2),b.hat,se.hat,max,min)
  omega.table=data.frame(omega)
  lambda.mat=lambda.mat
  A=A
  factor.mat=factor.mat
