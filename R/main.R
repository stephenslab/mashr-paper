...
get.prior.covar.Ukl <- function(P, lambda.mat, Q, factor.mat, omega.table) {
  ...
  for (l in 1:nrow(omega.table)) {
    ...
    cov.pc = 1/M * v[, 1:P] %*% diag(d[1:P]) %*% t(u[, 1:P]) %*% t(v[, 1:P] %*% diag(d[1:P]) %*% 
                                                                     t(u[, 1:P]))
...
post.array.generator = function(b.gp.hat, se.gp.hat, covmat) {
  ...
  for (j in 1:J) {
    ...
    for (k in 1:K) {
      ...
      for (r in 1:R) {
        ## ` marginal calculation for each tissue in each component if the covariance matrix has entry 0, then
        ## p(null=1)
