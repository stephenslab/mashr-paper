...
#'@title post.array.per.snp.with.lvlmat
#'@param cormat is the matrix of corerlation of errors
#'@param segphat is a the JxR matrix of standard errors for scaling
#'@export

post.array.per.snp.with.lvlmat=function(j,covmat,b.gp.hat,lvlmat){
  ...
  V.gp.hat=lvlmat###make sure to scale the correlation matrix of errors
  V.gp.hat.inv <- solve(V.gp.hat)
...
#'@title total.quant.per.snp.with.lvlmat
#'@param covmat a K list of covariance matrices
#'@param b.gp.hat JxR matrix of mles
#'@param se.gp.hat JxR matrix of standarderrors
#'@param cormat RxR matrix of variance
...
total.quant.per.snp.with.lvlmat=function(j,covmat,b.gp.hat,lvlmat,pis,A,checkpoint=FALSE){
  gene.snp.name=rownames(b.gp.hat)[j]
  V.gp.hat=lvlmat
  V.gp.hat.inv <- solve(V.gp.hat)
  b.mle=b.gp.hat[j,]
  ...
  all.arrays=post.array.per.snp.with.lvlmat(j=j,covmat=covmat,b.gp.hat=b.gp.hat,lvlmat = lvlmat)
  post.means=all.arrays$post.means
  post.ups=all.arrays$post.ups
  post.downs=all.arrays$post.downs
  post.covs=all.arrays$post.covs
...
#'@title total.quant.per.snp.with.lvlarray
#'@param covmat a K list of covariance matrices
#'@param b.gp.hat JxR matrix of mles
#'@param se.gp.hat JxR matrix of standarderrors
#'@param cormat RxR matrix of variance
...
total.quant.per.snp.with.lvlarray=function(j,covmat,b.gp.hat,lvlarray,pis,A,checkpoint=FALSE){
  gene.snp.name=rownames(b.gp.hat)[j]
  V.gp.hat=lvlarray[[j]]
  V.gp.hat.inv <- solve(V.gp.hat)
  b.mle=b.gp.hat[j,]
  ...
  all.arrays=post.array.per.snp.with.lvlmat(j=j,covmat=covmat,b.gp.hat=b.gp.hat,lvlmat = V.gp.hat)
  post.means=all.arrays$post.means
  post.ups=all.arrays$post.ups
  post.downs=all.arrays$post.downs
  post.covs=all.arrays$post.covs
...
#' @title compute.hm.train.log.lik.pen.lvlmat
...
compute.hm.train.log.lik.pen.lvlmat=function(train.b,lvlmat,covmat,A,pen){
  
  J=nrow(train.b)
  R=ncol(train.b)
  
  if(file.exists(paste0("liketrain",A,".rds"))==FALSE){
    lik.mat=t(sapply(seq(1:J),function(x){log.lik.func(b.mle=train.b[x,],V.gp.hat=lvlmat,covmat)}))##be sure to use log lik function
...
#' @title compute.hm.train.log.lik.pen.lvlarray
#' @description = takes a matrix of training sumamry statistics and their standard errors and computes the likelihood matrix according to a list of covariance matrices, using the exponent of the log likelihood - max (llj)
#' @param train.b =  JxR matrix of training beta hats
#' @param se.train = JxR matrix of training standard errors
#' @param covmat = LxK dimenstional (unlisted list) of prior covariance matrices
...
compute.hm.train.log.lik.pen.lvlarray=function(train.b,lvlarray,covmat,A,pen){
  
  J=nrow(train.b)
  R=ncol(train.b)
  
  if(file.exists(paste0("liketrain",A,".rds"))==FALSE){
    lik.mat=t(sapply(seq(1:J),function(x){log.lik.func(b.mle=train.b[x,],V.gp.hat=lvlarray[[x]],covmat)}))##be sure to use log lik function
