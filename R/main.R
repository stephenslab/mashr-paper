...
get.prior.covar.Ukl <- function(P, lambda.mat, Q, factor.mat, omega.table) {
  ...
  for (l in 1:nrow(omega.table)) {
    ...
    cov.pc <- 1/M * v[, 1:P] %*% diag(d[1:P]) %*% t(u[, 1:P]) %*% t(v[, 1:P] %*% 
      diag(d[1:P]) %*% t(u[, 1:P]))
...
mult.tissue.grid <- function(mult, betahat, sebetahat) {
  R <- ncol(betahat)
  mix.weights <- unlist(sapply(seq(1:ncol(b.gp.hat)), function(r) {
    autoselect.mix.sd(betahat = betahat[, r], sebetahat = sebetahat[, r], mult = 2)
  }))
  mult <- sqrt(2)
  sigmaamin <- min(mix.weights)
  sigmaamax <- max(mix.weights)
  npoint <- ceiling(log2(sigmaamax/sigmaamin)/log2(mult))
  omega <- mult^((-npoint):0) * sigmaamax
  return(omega)
}
...
lik.func <- function(b.mle, V.gp.hat, covmat) {
  sapply(seq(1:length(covmat)), function(x) {
    dmvnorm(x = b.mle, sigma = covmat[[x]] + V.gp.hat)
  })
}
...
post.weight.func <- function(pis, lik.mat) {
  d <- t(apply(lik.mat, 1, function(x) {
    x * pis$pihat
  }))
...
post.array.generator <- function(b.gp.hat, se.gp.hat, covmat) {
  ...
  for (j in 1:J) {
    ...
    for (k in 1:K) {
      ...
      for (r in 1:R) {
        ## ` marginal calculation for each tissue in each component if the covariance
        ## matrix has entry 0, then p(null=1)
        if (post.covs[j, k, r] == 0) {
          post.ups[j, k, r] <- 0  #need to figure out how to make the list have individual entries
          post.downs[j, k, r] <- 0
          post.nulls[j, k, r] <- 1
        } else {
          post.ups[j, k, r] <- pnorm(0, mean = mu.gp1kl[r], sd = sqrt(diag(U.gp1kl)[r]), 
          lower.tail = F)
          post.downs[j, k, r] <- pnorm(0, mean = mu.gp1kl[r], sd = sqrt(diag(U.gp1kl)[r]), 
          lower.tail = T)
          post.nulls[j, k, r] <- 0
        }
      }
...
compare.func <- function(j) {
  as.matrix(apply(rbind(all.upper[j, ], all.lower[j, ]), 2, function(j) {
    1 - max(j)
  }))
}
...
gen.mean <- function(post.means, post.weights, A) {
  all.mus <- t(sapply(seq(1:J), function(j) {
    total.mean(j)
  }))
...
gen.null.probs <- function(post.nulls, post.weights, A) {
  all.nuller <- t(sapply(seq(1:J), function(j) {
    total.null(j)
  }))
...
gen.tail.probs <- function(post.ups, post.downs, post.weights, A) {
  all.lower <- t(sapply(seq(1:J), function(j) {
    total.down(j)
  }))
  
  all.upper <- t(sapply(seq(1:J), function(j) {
    total.up(j)
  }))
  
  lfsr.mat <- t(sapply(seq(1:J), function(j) {
    compare.func(j)
  }))
...
compute.covmat <- function(b.gp.hat, sebetahat, Q, X.c, lambda.mat, P, A, factor.mat) {
  ...
  U.0kl <- get.prior.covar.Ukl(P = 2, lambda = lambda, Q = Q, factor.mat = factor.mat, 
    omega.table = omega.table)
...
compute.mixture.dist <- function(b.gp.hat, se.gp.hat, covmat, A = t.stat.inference) {
  ...
  lik.mat <- t(sapply(seq(1:J), function(x) {
    lik.func(b.mle = t.j[x, ], V.gp.hat = diag(v.j[x, ])^2, covmat)
  }))
