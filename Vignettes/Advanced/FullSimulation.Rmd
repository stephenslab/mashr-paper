...
---
 The purpose is to simulate according to the patterns of sharing present in the GTEX V6 Data, where 100 snps in cis of 500 genes, $pi0$ is 0.80, and there is sharing according to gtex covmats 2:9. It's critical to note that in the real GTEX data, 75% of the prior weight is on Uk2-9.
...
```{r, eval=FALSE}
##show that the two em calculations match##
compute.hm.train(train.b = t[1:20000,],se.train = v.j[1:20000,],covmat = covmat,A="Simulations")
compute.hm.train.log.lik(train.b = t[1:20000,],se.train = v.j[1:20000,],covmat = covmat,A = "Simulations.log.lik.only")
```
...
Let's compare the RMSE here with BMA:

```{r univariateashcompare}
bma.pm=as.matrix(read.table("~/Dropbox/testsimBMAonlyposterior.means.txt")[,-1])
...
png("~/matrix_ash/Vignettes/Figures/scatterplot.fitted.png")
...
points(as.matrix(t[null.vals,]),as.matrix(post.means[null.vals,]),col="red")
dev.off()

png("~/matrix_ash/Vignettes/Figures/scatterplot.fittedash.png")
...
png("~/matrix_ash/Vignettes/Figures/scatterplot.fittedbma.png")
plot(as.matrix(t[1:10000,]),as.matrix(bma.pm),col=adjustcolor("blue", alpha=0.5), pch=16,main="E(Z|Data) vs Zmle",ylab="E(Z|Data.bma)",xlab="Zmle")
points(t[null.vals,],bma.pm[null.vals,],col="red")
dev.off()


png("~/matrix_ash/Vignettes/Figures/scatterplot.truth.png")
...
png("~/matrix_ash/Vignettes/Figures/scatterplot.TRUTH.ash.png")
...
png("~/matrix_ash/Vignettes/Figures/scatterplot.TRUTH.bma.png")
plot(as.matrix(t.truth.test[1:10000,]),as.matrix(bma.pm),col=adjustcolor("blue", alpha=0.5), pch=16,main="E(Z|Data.bma) vs Z.True",ylab="E(Z|Data)",xlab="Z.True",ylim=c(-30,30))
points(t.truth.test[null.vals,],univariate.ash.pm[null.vals,],col="red")
legend("right",legend=paste0("RMSE",round(sqrt(mean((t.truth.test[1:10000,]-bma.pm)^2)),2)))
dev.off()

```

![alt text](Figures/scatterplotfitted.png) 
![alt text](Figures/scatterplotfittedash.png) 
![alt text](Figures/scatterplottruth.png)
![alt text](Figures/scatterplotTRUTHash.png)
