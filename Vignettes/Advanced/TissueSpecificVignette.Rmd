...
library("mash")
library("mvtnorm")
data=readRDS(system.file("simdata","simdata_tspec.rds", package = "mash"))
t=data$tstat;bhat=data$betahat;sebetahat=data$sebetahat;beta=data$beta;s.j=matrix
c=apply(t,1,function(x){max(abs(x))})
maxes=order(c,decreasing = TRUE)[1:400]##take top gene snp pairs
max.t=t[maxes,]
##sfa -gen ./maxt.txt -g 400 -n 44 -o tri_sim_spect i -k 5

A="yourfavoritename"
factor.mat=as.matrix(read.table(system.file("simdata","tri_sim_spect_F.out",package="mash")))
lambda.mat=as.matrix(read.table(system.file("simdata","tri_sim_spect_lambda.out",package ="mash")))
...
#Now, use compute.hm.train.log.lik for numerical issues

```

This will the object of pis, a pdf of their weight, and a likelihood matrix.
Now, we can compute posteriors. For simplicity, let's just do it for the first 5000 gene-snp pairs:

```{r computeposteriors,eval=FALSE}
A="yourfavoritename"
pis=readRDS(system.file("simdata","pisyourfavoritename.rds", package = "mash"))$pihat
...
A="yourfavoritename"
pis=readRDS(system.file("simdata","pisyourfavoritename.rds", package = "mash"))$pihat
covmat=readRDS(system.file("simdata","covmatyourfavoritename.rds", package = "mash"))
...
post.means=as.matrix(read.table(system.file("simdata","yourfavoritenameposterior.means.txt",package = "mash"))[,-1])
...
mash.means=as.matrix(read.table(system.file("simdata","yourfavoritenameposterior.means.txt",package = "mash"))[,-1])
univariate.ash.pm=read.table(system.file("simdata","uniashpm.txt",package = "mash"))
...
lfsr.mash=read.table(paste0("../simdata/",A,"lfsr.txt"))[,-1]
lfsr.ash=read.table("../simdata/uniashlfsr.txt")
...
for(thresh in seq(from = 0.01,to = 0.1,by = 0.001)){
points(thresh,sum(beta!=0&lfsr.ash<thresh),col="red",pch=3)}
