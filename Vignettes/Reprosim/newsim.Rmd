...
If you wanted to compute the denoised estimates using Extreme Deconvolution, you would need the Extreme Deconvolution package from Bovy et al as well:
```{r,eval=FALSE}
...
cov=compute.hm.covmat.all.max.step(max.step = ms,b.gp.hat = t,sebetahat = s.j,Q =5, t.stat=t[index,],lambda.mat=lambda.mat,P=3,A=A, factor.mat = factor.mat,bma = TRUE,zero=TRUE,power = 2)$cov
```
...
```{r}
mash.means=read.table("reproseedomega2posterior.means.txt")[,-1]
univariate.ash.pm=read.table("univariateashpm.txt")
lfsr.mash=read.table("reproseedomega2lfsr.txt")[,-1]
