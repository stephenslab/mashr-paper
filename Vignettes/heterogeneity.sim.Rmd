...
sebetahat=as.matrix(sebetahat)[1:10000,]
beta=as.matrix(beta)[1:10000,]

t.truth.test=beta/sebetahat
t.truth.test=as.matrix(t.truth.test)

mash.means=as.matrix(read.table("~/Dropbox/Simulationsposterior.means.txt")[,-1])
univariate.ash.pm=as.matrix(read.table("~/Dropbox/univariate.ash.pm.txt"))
bma.pm=as.matrix(read.table("~/Dropbox/testsimBMAonlyposterior.means.txt")[,-1])
beta=as.matrix(beta)[1:10000,]
t.truth.test=as.matrix(t.truth.test)[1:10000,]

mash.beta=mash.means*sebetahat
ash.beta=univariate.ash.pm*sebetahat
bma.beta=bma.pm*sebetahat
...
true.h=het.index(beta,thresh=0.5)
...
mash.h=het.index.two(mash.beta,0.5)
ash.h=het.index.two(ash.beta,0.5)
bma.h=het.index.two(bma.beta,0.5)

sqrt(mean(true.h-mash.h)^2)
sqrt(mean(true.h-ash.h)^2)
sqrt(mean(true.h-bma.h)^2)

sqrt(mean(true.h[1:400]-mash.h[1:400])^2)
sqrt(mean(true.h[1:400]-ash.h[1:400])^2)
sqrt(mean(true.h[1:400]-bma.h[1:400])^2)
```
...
mash.means=as.matrix(read.table("~/Dropbox/testtissuespecposterior.means.txt")[,-1])
univariate.ash.pm=as.matrix(read.table("~/Dropbox/univariate.ash.pm.tspec.txt"))
bma.pm=as.matrix(read.table("~/Dropbox/testimBMAonly_tspecposterior.means.txt")[,-1])
