...
v.j=s.j^2
covmat=compute.covmat(b.gp.hat = t,sebetahat = s.j,Q =5, t.stat=max.t,lambda.mat=lambda.mat,P=3,A="testim_tspec_new", factor.mat = factor.mat,bma = TRUE)##Note that here, we perform our inference including the BMA configs, se use the max.t to determine Uk and the overall set of t statistics and their standard errors to choose the grid).
```
...
library('mvtnorm')
covmat=readRDS("~/Dropbox/covmattestim_tspec.rds")
pis=readRDS("~/Dropbox/pistesttissuespec.rds")$pihat
...
pw.mat=matrix(post.weights,ncol = 54,byrow = T)
