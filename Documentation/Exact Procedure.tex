...
\section{Testing with Residuals}

Here, I use the covariance matrix of the residuals from the beta_hats, normalized such that the diagonal is 1 (because the variance of the Z statistic should be 1):

\begin{verbatim}
se.mat.train=cor(read.table("null.train.se.txt"))
se.mat.test=cor(read.table("null.test.se.txt"))



A="rank5withres"
covmat=compute.hm.covmat.all.max.step(z.stat,v.j,Q=5,lambda.mat,A,factor.mat,max.step=max.steprank5)

tim=proc.time()
compute.hm.train.semat(train.z,se.mat.train,covmat,A)
proc.time()-tim
...
compute.lik.test.semat(test.z,J=nrow(null.test),se.mat.test,covmat,A,pis) 
